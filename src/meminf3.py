...
class initialize(object):
    ...
    def get_layer_outputs(self, model, features):
        ...
        for l in self.layers_to_exploit:
            ...
            #print("第{}次把predicted加入inputArray################################:\n{}\n".format(i,self.inputArray))
            i+=1
    ...
    def get_gradients(self, model, features, labels):
        ...
        for s in splitted:
            ...
            #print("第{}次把gradient加入inputArray################################:\n{}\n".format(j,self.inputArray))
            j+=1
    ...
    def forward_pass(self, model, features, labels):
        ...
        tol_ias = self.wdi
        # tol_nias = self.wdni
        # fl_clients = Clients(input_shape=[None, num_input, num_input, num_input_channel],
        #               num_classes=num_classes,
        #               learning_rate=learning_rate,
        #               clients_num=1)
        #tol_ias,tol_nias = fl_clients.train_attack()

        for i in tol_ias:
            inputa = []
            #ti = np.array(i)
            for tensor_i in i:
                t = tf.convert_to_tensor(tensor_i)
                inputa.append(t)
            #print("\ninputarray_attack_outputs:\n", inputa)
            attack_outputs = self.attackmodel(inputa)
            print("attack_outputs:\n", attack_outputs)

        # for ni in tol_nias:
        #     #attack_outputs = self.attackmodel(ni)
        #     print("ninputarray_attack_outputs:\n", ni)
        ###############################################################################################################
    ...
    def train_attack(self):
            ...
            for e in range(self.epochs):
                ...
                for((mfeatures, mlabels), (nmfeatures, nmlabels)) in zipped:
                    with tf.GradientTape() as tape:
                        ...
                        #print("nmoutputs################################:{}".format(nmoutputs))
        ...
        # main training procedure ends
            ...
            for (input, ninput) in zip(self.wdi, self.wdni):
                inputa = []
                # ti = np.array(i)
                for tensor_i in input:
                    t = tf.convert_to_tensor(tensor_i)
                    inputa.append(t)
                ninputa = []
                # ti = np.array(i)
                for tensor_i in ninput:
                    t = tf.convert_to_tensor(tensor_i)
                    ninputa.append(t)
                mprobs = self.attackmodel(inputa)
                nmprobs = self.attackmodel(ninputa)
                probs = tf.concat((mprobs, nmprobs), 0)
                target_ones = tf.ones(mprobs.shape, dtype=bool)
                target_zeros = tf.zeros(nmprobs.shape, dtype=bool)
                target = tf.concat((target_ones, target_zeros), 0)
                attack_acc(probs > 0.4, target)
            result = attack_acc.result()
            print("攻击准确率：{}\n".format(result))
        data = None
